...
const REAL_API = "places/browse/";

const api = {
  getLocation: async (input = {}) => {
    if (MODE === "REAL_BACKEND") {
      const qs = new URLSearchParams(input).toString();
      const res = await authorizedFetch(qs ? `${REAL_API}?${qs}` : REAL_API);
    ...
    if (input.type) {
      data = data.filter((x) => x.type === input.type);
    }

    if (input.name) {
      data = data.filter((x) =>
        x.name.toLowerCase().includes(input.name.toLowerCase())
      );
    }
